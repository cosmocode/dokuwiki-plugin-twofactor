...
class action_plugin_twofactor_login extends DokuWiki_Action_Plugin
{
    ...
    public function twofactor_login_form(&$event, $param)
    {
        ...
        $twofa_form = form_makeTextField('otp', '', $this->getLang('twofactor_login'), '', 'block',
    ...
    public function twofactor_action_process_handler(&$event, $param)
    {
        ...
        $must_login = $mandatory || ($this->attribute->get("twofactor",
                    "state") == 'in' && $INPUT->server->str('REMOTE_USER', ''));
    ...
    private function _logout()
    {
        ...
        if ($this->attribute) {
            ...
            // Wipe out 2FA cookie.
            $this->log('del cookies: ' . TWOFACTOR_COOKIE . ' ' . print_r(headers_sent(), true),
                self::LOGGING_DEBUGPLUS);
            $cookie = '';
            $cookieDir = empty($conf['cookiedir']) ? DOKU_REL : $conf['cookiedir'];
    ...
    public function get_clearance($user = null)
    {
        ...
        if (!$clearance) {
            ...
            foreach (array_filter($this->attribute->enumerateAttributes("twofactor", $user), function ($key) {
                substr($key, 0, 3) == 'id.';
            }) as $attr) {
            ...
            if ($clearance) {
                $this->log("get_clearance: 2FA revived by cookie. Expiry: " . print_r($expiry,
                        true) . " Session: " . print_r($_SESSION, true), self::LOGGING_DEBUGPLUS);
            }
        }
    ...
    private function _grant_clearance($user = null)
    {
        ...
        if (!headers_sent()) {
            ...
            $this->log('_grant_clearance: conf:' . $this->getConf('loginnotice') . ' user:' . ($this->attribute->get("twofactor",
                    "loginnotice", $user) === true ? 'true' : 'false'), self::LOGGING_DEBUG);
            $send_wanted = $this->getConf('loginnotice') == 'always' || ($this->getConf('loginnotice') == 'user' && $this->attribute->get("twofactor",
                        "loginnotice", $user) == true);
            if ($send_wanted) {
        ...
        $this->attribute->set("twofactor", $key, $_SESSION[DOKU_COOKIE]['twofactor_clearance'], $user);
        // Set the 2FA cookie.
        $this->log('_grant_clearance: new cookies: ' . TWOFACTOR_COOKIE . ' ' . print_r(headers_sent(), true),
            self::LOGGING_DEBUGPLUS);
        $cookieDir = empty($conf['cookiedir']) ? DOKU_REL : $conf['cookiedir'];
    ...
    private function _send_login_notification()
    {
        ...
        $module = $this->attribute->exists("twofactor", "defaultmod") ? $this->attribute->get("twofactor",
            "defaultmod") : null;
    ...
    function twofactor_before_auth_check(&$event, $param)
    {
        ...
        $available = count($this->tokenMods) + count($this->otpMods) > 0;
        $this->log('twofactor_before_auth_check: Tokens:' . count($this->tokenMods) . ' Codes:' . count($this->otpMods) . " Available:" . (int)$available,
            self::LOGGING_DEBUGPLUS);
        if (!$available) {
        ...
        if ($ACT == 'login') {
            ...
            if ($INPUT->server->str('REMOTE_USER', '') == '') {
                ...
                if ($event->data['user']) {
                    // Send loglog an event to show the user logged in but needs OTP code.
                    $log = array(
                        'message' => 'logged in, ' . $this->getLang('requires_otp'),
    ...
    private function _process_changes(&$event, $param)
    {
        ...
        $sendotp = null;
        foreach ($this->modules as $name => $mod) {
            $this->log('_process_changes: processing ' . get_class($mod) . '::processProfileForm()',
                self::LOGGING_DEBUG);
            $result = $mod->processProfileForm();
            $this->log('_process_changes: processing ' . get_class($mod) . '::processProfileForm() == ' . $result,
                self::LOGGING_DEBUGPLUS);
            ...
            switch ((string)$result) {
                case 'verified':
                    ...
                    $this->_setHelperVariables();
                    $this->log("2FA Added: " . $INPUT->server->str('REMOTE_USER', '') . ' ' . get_class($mod),
                        self::LOGGING_AUDIT);
                    break;
                case 'failed':
                ...
                case 'otp':
                    ...
                    break;
                case 'deleted':
                    $this->log("2FA Removed: " . $INPUT->server->str('REMOTE_USER', '') . ' ' . get_class($mod),
                        self::LOGGING_AUDIT);
                    // Reset helper variables.
                    $this->_setHelperVariables();
                    break;
    ...
    function twofactor_otp_login(&$event, $param)
    {
        ...
        $form->addElement(form_makeTextField('otpcode', '', $this->getLang('twofactor_otplogin'), '', 'block',
            array('size' => '50', 'autocomplete' => 'off')));
    ...
    private function _send_message($subject, $message, $module = null, $force = false)
    {
        ...
        foreach ($module as $mod) {
            if ($mod->canTransmitMessage()) {
                ...
                $this->log("Message " . ($worked ? '' : 'not ') . "sent to $user via " . get_class($mod),
                    self::LOGGING_AUDITPLUS);
            }
        }
    ...
    public function log($message, $level = 1)
    {
        ...
        if ($level < self::LOGGING_AUDIT || $level > $this->getConf('logging_level')) {
            return;
        }
        global $conf;
        // Always purge line containing "[pass]".
        $message = implode("\n", array_filter(explode("\n", $message), function ($x) {
            return !strstr($x, '[pass]');
        }));
